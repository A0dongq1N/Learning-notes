...
- 问题现象：明明指定的服务器端口是`1234`为什么输出却成了`53764`?

  ![1555428761850](E:\Learning-notes\网络编程\assets\1555428761850.png)
...
int main(int argc, char* argv[])
{
	...
	server_in.sin_port = htons(1234);  //服务器的端口号
	server_in.sin_addr.S_un.S_addr = inet_addr("192.168.1.111"); //服务IP
...
- 问题原因：
  ...
  htons：将主机字节顺序转为网络字节顺序(大端模式)，此时`server_in.sin_port`已经转变为大端模式了，如果直接输入肯定是错误的，所以，需要进行网络字节顺序到主机字节顺序的转化，此时正确的写法如下：
